...
// Cross-cast to sri::CountIndex<> so the same lambda works whether t_idx.idx is a
// shared_ptr<LocateIndex<>> or shared_ptr<CountIndex<>>.
...
int main(int argc, char* argv[]) {
  ...
  // Count-only path: loads alphabet + psi-RLE only.
  {
    auto count_only = factory.makeCount();
    benchmark::RegisterBenchmark("R-CSA-Count", BM_QueryCount, count_only, patterns, n);
    if (FLAGS_print_result) {
      benchmark::RegisterBenchmark("Print-R-CSA-Count", BM_PrintQueryCount, "R-CSA-Count", count_only, patterns, n);
    }
  }
