...
class CSA {
 public:
  ...
  void load(const sdsl::cache_config &t_config) {
    ...
    auto cref_cumulative = std::cref(std::any_cast<const TAlphabet &>(it_alphabet->second).C);
    auto cumulative = sri::RandomAccessForCRefContainer(cref_cumulative);
    auto n = cref_cumulative.get()[cref_cumulative.get().size() - 1];
...
//    auto it_bv_mark = load<TBVMark>(sri::key_trait<t_width>::KEY_BWT_RUN_LAST, t_config, true);
    ...
    auto it_sample_index = load < TMarkToSampleIdx > (sri::key_trait<t_width>::KEY_BWT_RUN_LAST_TEXT_POS_TO_FIRST_IDX,
        t_config);
    auto get_mark_to_sample_idx = sri::RandomAccessForTwoContainersDefault(
        std::cref(std::any_cast<const TMarkToSampleIdx &>(it_sample_index->second)),
        true);

    auto it_sample = load < TSample > (sri::key_trait<t_width>::KEY_BWT_RUN_FIRST_TEXT_POS, t_config);
    auto get_sample = sri::RandomAccessForCRefContainer(std::cref(std::any_cast<const TSample &>(it_sample->second)));
    sri::SampleValidatorDefault sample_validator_default;

    auto phi = sri::buildPhiForward(successor, get_mark_to_sample_idx, get_sample, sample_validator_default, n);
...
//    auto it_bv_sample_idx = load < TBVSampleIdx > (sri::key_trait<t_width>::KEY_BWT_RUN_FIRST, t_config, true);
    ...
    auto get_sample_from_sa_pos = sri::buildGetSampleForBWTRun(
        std::cref(std::any_cast<const typename TBVSampleIdx::rank_1_type &>(it_bv_sample_idx_rank->second)),
        get_sample);

    auto compute_toehold = sri::buildComputeToeholdValueForPhiForward(cref_psi_core, get_sample_from_sa_pos);
    ...
    auto get_initial_data_backward_search_step = sri::GetInitialDataBackwardSearchStep(
        cref_psi_core.get().getFirstBWTSymbol(), n - 1);
 ...
 private:
  ...
  auto load(const std::string &t_key, const sdsl::cache_config &t_config, bool t_add_type_hash = false) {
    auto it = storage_.get().find(sdsl::cache_file_name(t_key, t_config));
  ...
  auto loadBVRank(const std::string &t_key, const sdsl::cache_config &t_config, bool t_add_type_hash = false) {
    ...
    auto it = storage_.get().find(sdsl::cache_file_name(key_rank, t_config));
    if (it == storage_.get().end()) {
      auto it_bv = load<TData>(t_key, t_config, t_add_type_hash);

      typename TData::rank_1_type rank(&std::any_cast<const TData &>(it_bv->second));
  ...
  auto loadBVSelect(const std::string &t_key, const sdsl::cache_config &t_config, bool t_add_type_hash = false) {
    ...
    auto it = storage_.get().find(sdsl::cache_file_name(key_select, t_config));
    if (it == storage_.get().end()) {
      auto it_bv = load<TData>(t_key, t_config, t_add_type_hash);

      typename TData::select_1_type select(&std::any_cast<const TData &>(it_bv->second));
