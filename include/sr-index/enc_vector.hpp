...
/*! \file enc_vector.hpp
   ...
   \author Simon Gog
*/
#ifndef SRI_ENC_VECTOR
#define SRI_ENC_VECTOR
...
struct enc_vector_trait<32> {
  typedef sdsl::int_vector<32> int_vector_type;
};

template<>
struct enc_vector_trait<64> {
  typedef sdsl::int_vector<64> int_vector_type;
};
...
class enc_vector
{
    ...
    public:
        ...
        typedef sdsl::random_access_const_iterator<enc_vector> iterator;
        ...
        typedef sdsl::iv_tag                                   index_category;
        static  const uint32_t                           sample_dens    = t_dens;
        typedef enc_vector                               enc_vec_type;

        sdsl::int_vector<0>     m_z;                       // storage for encoded deltas
    private:
    ...
    public:
        ...
        */
        ...
        enc_vector(sdsl::int_vector_buffer<int_width>& v_buf);
        ...
        {
          return sdsl::int_vector<>::max_size()/2;
        }
...
//    (2) Write sample values and deltas
    {
        if (max_sample_value > z_size+1)
          m_sample_vals_and_pointer.width(sdsl::bits::hi(max_sample_value) + 1);
        else
          m_sample_vals_and_pointer.width(sdsl::bits::hi(z_size+1) + 1);
        m_sample_vals_and_pointer.resize(2*samples+2); // add 2 for last entry
        sdsl::util::set_to_value(m_sample_vals_and_pointer, 0);
        ...
        *sv_it = z_size+1; ++sv_it; // last entry
        ...
        m_z = sdsl::int_vector<>(z_size, 0, 1);
...
template<class t_coder, uint32_t t_dens, uint8_t t_width>
template<uint8_t int_width>
enc_vector<t_coder, t_dens,t_width>::enc_vector(sdsl::int_vector_buffer<int_width>& v_buf)
{
    // clear bit_vectors
    clear();
...
template<class t_coder, uint32_t t_dens, uint8_t t_width>
    enc_vector<>::size_type enc_vector<t_coder, t_dens,t_width>::serialize(std::ostream& out, sdsl::structure_tree_node* v, std::string name)const
{
