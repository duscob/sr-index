...
template <class sparse_bitvector_t = sparse_sd_vector,  // predecessor structure storing run length
 ...
 public:
  ...
  ulint print_space() const {
...

    {
      ...
      for (auto r : runs_per_letter)
        bytesize += r.serialize(out);
...
class RLEString {
 public:
  ...
  auto rank(std::size_t t_i, const typename TString::value_type& t_c) const {
    ...
    if (runs_per_symbol.data.size() == 0)
      return 0ul;  // letter does not exist in the text
    if (t_i == size())
      return runs_per_symbol.data.size();
