...
template<typename TBackwardNav, typename TGetLastValue, typename TComputeAllValues>
class RIndex : public LocateIndex {
 public:
  ...
  void Locate(const TPattern &t_pattern, TReport &t_report) const {
    ...
    auto last_value = std::experimental::make_optional(final_value_);

    for (auto it = rbegin(t_pattern); it != rend(t_pattern) && range.first <= range.second; ++it) {
      auto c = *it;

      auto next_range = lf_(range, c);
      last_value = get_last_value_(range, next_range, c, last_value);
...
auto buildRIndex(const TBackwardNav &t_lf,
                 ...
                 std::size_t t_bwt_size,
                 std::size_t t_final_sa_value) {
  return RIndex<TBackwardNav, TGetLastValue, TComputeAllValues>(
      t_lf, t_get_last_value, t_compute_all_values, t_bwt_size, t_final_sa_value);
}
...
auto buildSharedPtrRIndex(const TBackwardNav &t_lf,
                          ...
                          std::size_t t_bwt_size,
                          std::size_t t_final_sa_value) {
  return std::make_shared<RIndex<TBackwardNav, TGetLastValue, TComputeAllValues>>(
      t_lf, t_get_last_value, t_compute_all_values, t_bwt_size, t_final_sa_value);
}
